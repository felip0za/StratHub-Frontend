  ...
  const carregarRanking = async () => {
    setLoading(true);
    await buscarDados(); // reutiliza lógica de dados
    setLoading(false);
  };

  const atualizarRankingSilenciosamente = async () => {
    await buscarDados(); // sem alterar o loading
  };
  ...
  const buscarDados = async () => {
    try {
      ...
      usuarioTime.pontuacao = resPontuacao.data.pontuacao || 0;
  ...
  // Atualizações silenciosas a cada 10 segundos
  intervalo = setInterval(() => {
    atualizarRankingSilenciosamente();
  }, 1000);
